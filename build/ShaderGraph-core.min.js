...
b,f){c.Block.Snippet.compileCall(a,b,this.node(),this.snippet,f)},fetch:function(a,b,f,c){a.include(this,b)||this.insert(a,b,c);return f.id()},outlets:function(){return c.Block.Snippet.makeOutlets(this.snippet)}});c.Block.Material=function(a,b){this.vertex=new c.Snippet(a);this.fragment=new c.Snippet(b);c.Block.call(this)};c.Block.Material.prototype=_.extend({},c.Block.prototype,{compile:function(){if(0<this.node().outputs.length)throw"Can't compile material with outputs";this.node();var a=new c.Program;
this.insert(a,"vertex",0);this.insert(a,"fragment",0);a.compile();return a},insert:function(a,b,f){c.Block.Snippet.compileCall(a,b,this.node(),this[b],f)},fetch:function(a,b,c,d){a.include(this,b)||this.insert(a,b,d);"fragment"==b&&(a.include(this,"vertex")||this.insert(a,"vertex",0));return c.id()},outlets:function(){var a=c.Block.Snippet.makeOutlets(this.vertex),b=c.Block.Snippet.makeOutlets(this.fragment);return _.union(a,b)}});c.Block.Snippet.makeOutlets=function(a){var b=[];if(a.outlets)return a.outlets;
var f=a.arguments();_.each(f.parameters,function(a){a.meta={required:!0};a.hint=a.name.replace(/(In|Out)$/,"");a.category="parameter";b.push(a)});_.each(f.uniforms,function(a){a.meta={};a.hint=a.name.replace(/(In|Out)$/,"");a.category="uniform";a.inout=c.IN;b.push(a)});return a.outlets=b};c.Block.Snippet.compileCall=function(a,b,f,d,g){var e=d.arguments(),h=[];_.each(e.parameters,function(e){var d=f.get(e.name);if(e.inout==c.IN)if(d.input)d=d.input.node.owner().fetch(a,b,d.input,g+1),a.variable(b,
d,e),h.push(d);else throw console.log("Outlet",e,d),["Missing connection on outlet for "+e.name];else e.inout==c.OUT&&(d=d.id(),a.variable(b,d,e),h.push(d))});var i=[];_.each(e.uniforms,function(c){var e=f.get(c.name);e.input?(e=e.input.node.owner().fetch(a,b,e.input,g+1),a.variable(b,e,c),h.push(e),i.push(c.name)):a.external("uniform",c)});_.each(e.attributes,function(b){a.external("attribute",b)});_.each(e.varyings,function(b){a.external("varying",b)});e=["_sg",b,d.name,f.owner().index].join("_");
d=d.compile(e,i,!0);a.add(b,e,h,d,g)}})(ShaderGraph);
...
b);this.externals[b.name]=b},variable:function(a,b,c){c=_.extend({},c,{name:b});this.variables[a][b]=c},add:function(a,b,c,d,g){var e=this.calls[a][b];e?e.priority=Math.min(e.priority,g):this.calls[a][b]={name:b,args:c,code:d,priority:g}},compile:function(){_.each(this.externals,function(a){"uniform"==a.category&&(this.uniforms[a.name]={type:a.type,value:a.value});"attribute"==a.category&&(this.attributes[a.name]={type:a.type,value:[]})}.bind(this));_.each(["vertex","fragment"],function(a){var b=
[];_.each(this.externals,function(c){"attribute"==c.category&&"fragment"==a||b.push([c.category,c.signature,";"].join(" "))}.bind(this));var b=b.join("\n"),f=_.toArray(this.calls[a]),d=[b],g=["void main() {"];_.each(this.variables[a],function(a){g.push([c.Program.types[a.type],a.name,";"].join(" "))});f.sort(function(a,b){return b.priority-a.priority});_.each(f,function(a){d.push(a.code);g.push([a.name,"(",a.args.join(","),");"].join(""))});g.push("}");this[a+"Shader"]=[d.join("\n"),g.join("\n")].join("\n")}.bind(this));
...
b){if(!a.global)throw"Can't findAll non-global regexp";for(var c,f=[];c=a.exec(b);)f.push(c);return f}var a=a.replace(/\r\n?/g,"\n").replace(/\/\/[^\n]*\n/g," ").replace(/\/\*(.|\n)*?\*\//g," "),c=b(/(?:^|;)\s*attribute\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),d=b(/(?:^|;)\s*uniform\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),g=b(/(?:^|;)\s*varying\s+(([A-Za-z0-9]+)\s+([[A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),e=b(/(?:^|;)\s*void\s+([A-Za-z0-9]+)\s*\(([^\)]*)\)\s*{/g,
a);if(!e[0])throw"Could not parse shader snippet. Must contain a void-returning function with in/outs: "+a;var h=a;_.each({parseAttribute:c,parseUniform:d,parseVarying:g},function(a,b){_.each(a,function(a){this[b](a);h=h.replace(a[0],"")}.bind(this))}.bind(this));h=h.replace(/^\s*;/,"");this.parseSignature(e[0]);this.body=h}}})(ShaderGraph);
...
a.link(this);outlets[b]=a;(a.inout==c.IN?_in:_out).push(a);return this},remove:function(a){var b=this._outlets,f=this.key(a),d=a.inout==c.IN?this.inputs:this.outputs;if(a.node!=this)throw"Removing outlet from wrong node.";a.disconnect();a.link(null);delete b[f];d.splice(d.indexOf(a),1);return this},connect:function(a,b,c){var d={},g={},e;e={};_.each(a.inputs,function(a){if(c||!a.input){var b=a.type,j=[b,a.hint].join("-"),k=e[b]=(e[b]||0)+1,b=[b,k].join("-");g[j]=a;d[b]=a}});e={};_.each(this.outputs,
function(a){if(!b||!a.output.length){var c=a.type,f=[c,a.hint].join("-");g[f]?g[f].connect(a):(f=e[c]=(e[c]||0)+1,c=[c,f].join("-"),d[c]&&d[c].connect(a))}});return this},disconnect:function(){_.each(this.inputs,function(a){a.disconnect()});_.each(this.outputs,function(a){a.disconnect()});return this}}})(ShaderGraph);
(function(c){c.Outlets=0;c.Outlet=function(a,b,f,d,g,e,h){if("object"==typeof a)return new c.Outlet(a.inout,a.name,a.hint,a.type,a.category,a.exposed);this.node=null;this.inout=a;this.name=b;this.hint=f||b;this.type=d;this.category=g;this.exposed=!!e;this.meta=h||{};this.index=++c.Outlets;this.input=null;this.output=[]};c.Outlet.prototype={id:function(){return["_sg",this.name,this.index].join("_")},expose:function(a){this.exposed=a},morph:function(a){this.inout=a.inout;this.name=a.name;this.type=
