...
d,f),h.push(d);else throw console.log("Outlet",f,d),["Missing connection on outlet for "+f.name];else f.inout==c.OUT&&(d=d.id(),a.variable(b,d,f),h.push(d))});var j=[];_.each(f.uniforms,function(c){var f=e.get(c.name);f.input?(f=f.input.node.owner().fetch(a,b,f.input,g+1),a.variable(b,f,c),h.push(f),j.push(c.name)):a.external("uniform",c)});_.each(f.attributes,function(b){a.external("attribute",b)});_.each(f.varyings,function(b){a.external("varying",b)});f=["_sg",b,d.name,e.owner().index].join("_");
d=d.compile(f,j,!0);a.add(b,f,h,d,g)}})(ShaderGraph);
