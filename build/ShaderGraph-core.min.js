...
var e=a.arguments();_.each(e.parameters,function(a){a.meta={required:!0};a.hint=a.name.replace(/(In|Out)$/,"");a.category="parameter";b.push(a)});_.each(e.uniforms,function(a){a.meta={};a.hint=a.name.replace(/(In|Out)$/,"");a.category="uniform";a.inout=c.IN;b.push(a)});return a.outlets=b};c.Block.Snippet.compileCall=function(a,b,e,d,g){var f=d.arguments(),h=[];_.each(f.parameters,function(f){var d=e.get(f.name);if(f.inout==c.IN)if(d.input)d=d.input.node.owner().fetch(a,b,d.input,g+1),a.variable(b,
d,f),h.push(d);else throw console.log("Outlet",f,d),["Missing connection on outlet for "+f.name];else f.inout==c.OUT&&(d=d.id(),a.variable(b,d,f),h.push(d))});var i=[];_.each(f.uniforms,function(c){var f=e.get(c.name);f.input?(f=f.input.node.owner().fetch(a,b,f.input,g+1),a.variable(b,f,c),h.push(f),i.push(c.name)):a.external("uniform",c)});_.each(f.attributes,function(b){a.external("attribute",b)});_.each(f.varyings,function(b){a.external("varying",b)});f=["_sg",b,d.name,e.owner().index].join("_");
d=d.compile(f,i,!0);a.add(b,f,h,d,g)}})(ShaderGraph);
...
b){if(!a.global)throw"Can't findAll non-global regexp";for(var c,e=[];c=a.exec(b);)e.push(c);return e}var a=a.replace(/\r\n?/g,"\n").replace(/\/\/[^\n]*\n/g," ").replace(/\/\*(.|\n)*?\*\//g," "),c=b(/(?:^|;)\s*attribute\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),d=b(/(?:^|;)\s*uniform\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),g=b(/(?:^|;)\s*varying\s+(([A-Za-z0-9]+)\s+([[A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),f=b(/(?:^|;)\s*void\s+([A-Za-z0-9]+)\s*\(([^\)]*)\)\s*{/g,
...
a.link(this);outlets[b]=a;(a.inout==c.IN?_in:_out).push(a);return this},remove:function(a){var b=this._outlets,e=this.key(a),d=a.inout==c.IN?this.inputs:this.outputs;if(a.node!=this)throw"Removing outlet from wrong node.";a.disconnect();a.link(null);delete b[e];d.splice(d.indexOf(a),1);return this},connect:function(a,b,c){var d={},g={};_.each(a.inputs,function(a){if(c||!a.input){var b=a.type,i=[b,a.hint].join("-");g[i]||(g[i]=a);d[b]=d[b]||[];d[b].push(a)}});_.each(this.outputs,function(a){if(!b||
!a.output.length){var c=a.type,e=[c,a.hint].join("-");g[e]?(g[e].connect(a),delete g[e],d[c].splice(d[c].indexOf(a),1)):d[c]&&d[c].length&&d[c].shift().connect(a)}});return this},disconnect:function(){_.each(this.inputs,function(a){a.disconnect()});_.each(this.outputs,function(a){a.disconnect()});return this}}})(ShaderGraph);
