...
a.inout=c.IN;b.push(a)});return a.outlets=b};c.Block.Snippet.compileCall=function(a,b,e,d,g){var f=d.arguments(),h=[];_.each(f.parameters,function(f){var d=e.get(f.name,f.inout==c.INOUT?c.IN:f.inout);if(f.inout==c.IN||f.inout==c.INOUT)if(d.input)d=d.input.node.owner().fetch(a,b,d.input,g+1),a.variable(b,d,f),h.push(d);else throw console.log("Outlet",f,input),["Missing connection on outlet for "+f.name];else f.inout==c.OUT&&(d=d.id(),a.variable(b,d,f),h.push(d))});var j=[];_.each(f.uniforms,function(c){var f=
e.get(c.name);f.input?(f=f.input.node.owner().fetch(a,b,f.input,g+1),a.variable(b,f,c),h.push(f),j.push(c.name)):a.external("uniform",c)});_.each(f.attributes,function(b){a.external("attribute",b)});_.each(f.varyings,function(b){a.external("varying",b)});f=["_sg",b,d.name,e.owner().index].join("_");d=d.compile(f,j,!0);a.add(b,f,h,d,g)}})(ShaderGraph);
...
b.start.concat(a.start);b.end=b.end.concat(a.end);this.stack.unshift({start:[],end:[]});return this},combine:function(){if(2>=this.stack.length)throw"Popping factory stack too far.";this.next();this.stack.shift();var a=this.stack.shift(),b=this.stack[0];a.start.length&&(_.each(a.start,function(c){c?_.each(b.end,function(a){a.connect(c,!0)}):a.end=a.end.concat(b.end)}),b.end=a.end);return this},end:function(){var a=this.graph;this.graph=new c.Graph;this.stack=[];this.group();a&&(a.compile=function(){return a.tail().owner().compile()});
