...
b,e){c.Block.Snippet.compileCall(a,b,this.node(),this.snippet,e)},fetch:function(a,b,e,c){a.include(this,b)||this.insert(a,b,c);return e.id()},outlets:function(){return c.Block.Snippet.makeOutlets(this.snippet.arguments())}});c.Block.Material=function(a,b){this.vertex=new c.Snippet(a);this.fragment=new c.Snippet(b);c.Block.call(this)};c.Block.Material.prototype=_.extend({},c.Block.prototype,{compile:function(){if(0<this.node().outputs.length)throw"Can't compile material with outputs";this.node();
var a=new c.Program;this.insert(a,"vertex",0);this.insert(a,"fragment",0);a.compile();return a},insert:function(a,b,e){c.Block.Snippet.compileCall(a,b,this.node(),this[b],e)},fetch:function(a,b,e,c){a.include(this,b)||this.insert(a,b,c);"fragment"==b&&(a.include(this,"vertex")||this.insert(a,"vertex",0));return e.id()},outlets:function(){var a=c.Block.Snippet.makeOutlets(this.vertex.arguments()),b=c.Block.Snippet.makeOutlets(this.fragment.arguments());return _.union(a,b)}});c.Block.Snippet.makeOutlets=
function(a){var b=[];_.each(a.parameters,function(a){a.meta={required:!0};a.hint=a.name.replace(/(In|Out)$/,"");a.category="parameter";b.push(a)});_.each(a.uniforms,function(a){a.meta={};a.hint=a.name.replace(/(In|Out)$/,"");a.category="uniform";a.inout=c.IN;b.push(a)});return b};c.Block.Snippet.compileCall=function(a,b,e,d,g){var f=d.arguments(),h=[];_.each(f.parameters,function(d){var f=e.get(d.name);if(d.inout==c.IN)if(f.input)f=f.input.node.owner().fetch(a,b,f.input,g+1),a.variable(b,f,d.type),
h.push(f);else throw console.log("Outlet",d,f),["Missing connection on outlet for "+d.name];else d.inout==c.OUT&&(f=f.id(),a.variable(b,f,d.type),h.push(f))});var i=[];_.each(f.uniforms,function(c){var d=e.get(c.name);d.input?(d=d.input.node.owner().fetch(a,b,d.input,g+1),a.variable(b,d,c.type),h.push(d),i.push(c.name)):a.external("uniform",c.name,c.type,c.value)});_.each(f.attributes,function(b){a.external("attribute",b.name,b.type)});f=["_sg",b,d.name,e.owner().index].join("_");d=d.compile(f,i);
a.add(b,f,h,d,g)}})(ShaderGraph);
(function(c){c.Factory=function(){this.end()};c.Factory.prototype={snippet:function(a,b){var b=b||"append",e=new c.Block.Snippet(ShaderGraph.getShader(a));this[b](e.node());return this},material:function(a,b,e){e=e||"append";a=new c.Block.Material(ShaderGraph.getShader(a),ShaderGraph.getShader(b));this[e](a.node());return this},snippetBefore:function(a){this.snippet(a,"prepend");return this},materialBefore:function(){this.material(code,"prepend");return this},append:function(a){a.graph||this.graph.add(a);
...
a[1];var b=a[2].replace(/^\s*$/g,"");0==b.length?this.signature=[]:(arguments=this.signature=b.split(","),_.each(arguments,function(a){var a=/((?:(in|out|inout)\s+)?([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/.exec(a),b=a[1];this.parameters.push({inout:{"in":c.IN,out:c.OUT,inout:c.INOUT}[a[2]||"in"],name:a[4],type:this.type(a[3],a[5]),signature:b})}.bind(this)))},parseCode:function(a){function b(a,b){if(!a.global)throw"Can't findAll non-global regexp";for(var c,e=[];c=a.exec(b);)e.push(c);
return e}var a=a.replace(/\r\n?/g,"\n").replace(/\/\/[^\n]*\n/g," ").replace(/\/\*(.|\n)*?\*\//g," "),c=b(/(?:^|;)\s*attribute\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),d=b(/(?:^|;)\s*uniform\s+(([A-Za-z0-9]+)\s+([A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),g=b(/(?:^|;)\s*varying\s+(([A-Za-z0-9]+)\s+([[A-Za-z0-9_]+)\s*(?:\[([^\]]+)\])?)(?:$|(?=;))/g,a),f=b(/(?:^|;)\s*void\s+([A-Za-z0-9]+)\s*\(([^\)]*)\)\s*{/g,a);if(!f[0])throw"Could not parse shader snippet. Must contain a void-returning function with in/outs: "+
a;var h=a;_.each({parseAttribute:c,parseUniform:d,parseVarying:g},function(a,b){_.each(a,function(a){this[b](a);h=h.replace(a[0],"")}.bind(this))}.bind(this));h=h.replace(/^\s*;/,"");this.parseSignature(f[0]);this.body=h}}})(ShaderGraph);
...
a.link(this);outlets[b]=a;(a.inout==c.IN?_in:_out).push(a);return this},remove:function(a){var b=this._outlets,e=this.key(a),d=a.inout==c.IN?this.inputs:this.outputs;if(a.node!=this)throw"Removing outlet from wrong node.";a.disconnect();a.link(null);delete b[e];d.splice(d.indexOf(a),1);return this},connect:function(a,b,c){var d={},g={},f;f={};_.each(a.inputs,function(a){if(c||!a.input){var b=a.type,j=[b,a.hint].join("-"),k=f[b]=(f[b]||0)+1,b=[b,k].join("-");g[j]=a;d[b]=a}});f={};_.each(this.outputs,
function(a){if(!b||!a.output.length){var c=a.type,e=[c,a.hint].join("-");g[e]?g[e].connect(a):(e=f[c]=(f[c]||0)+1,c=[c,e].join("-"),d[c]&&d[c].connect(a))}});return this},disconnect:function(){_.each(this.inputs,function(a){a.disconnect()});_.each(this.outputs,function(a){a.disconnect()});return this}}})(ShaderGraph);
