...
c.bind(d[a],d)});return d};c.memoize=function(a,b){var e={};b||(b=c.identity);return function(){var f=b.apply(this,arguments);return c.has(e,f)?e[f]:e[f]=a.apply(this,arguments)}};c.delay=function(a,c){var b=g.call(arguments,2);return setTimeout(function(){return a.apply(null,b)},c)};c.defer=function(a){return c.delay.apply(c,[a,1].concat(g.call(arguments,1)))};c.throttle=function(a,b){var e,f,g,h,k,i,n=c.debounce(function(){k=h=!1},b);return function(){e=this;f=arguments;g||(g=setTimeout(function(){g=
...
b[c]});return a};c.pick=function(a){var b={};l(c.flatten(g.call(arguments,1)),function(c){c in a&&(b[c]=a[c])});return b};c.defaults=function(a){l(g.call(arguments,1),function(b){for(var c in b)null==a[c]&&(a[c]=b[c])});return a};c.clone=function(a){return!c.isObject(a)?a:c.isArray(a)?a.slice():c.extend({},a)};c.tap=function(a,c){c(a);return a};c.isEqual=function(a,c){return f(a,c,[])};c.isEmpty=function(a){if(null==a)return!0;if(c.isArray(a)||c.isString(a))return 0===a.length;for(var b in a)if(c.has(a,
...
var e=a.arguments();_.each(e.parameters,function(a){a.meta={required:!0};a.hint=a.name.replace(/(In|Out)$/,"");a.category="parameter";b.push(a)});_.each(e.uniforms,function(a){a.meta={};a.hint=a.name.replace(/(In|Out)$/,"");a.category="uniform";a.inout=f.IN;b.push(a)});return a.outlets=b};f.Block.Snippet.compileCall=function(a,b,e,h,i){var g=h.arguments(),m=[];_.each(g.parameters,function(g){var h=e.get(g.name);if(g.inout==f.IN)if(h.input)h=h.input.node.owner().fetch(a,b,h.input,i+1),a.variable(b,
h,g),m.push(h);else throw console.log("Outlet",g,h),["Missing connection on outlet for "+g.name];else g.inout==f.OUT&&(h=h.id(),a.variable(b,h,g),m.push(h))});var k=[];_.each(g.uniforms,function(f){var g=e.get(f.name);g.input?(g=g.input.node.owner().fetch(a,b,g.input,i+1),a.variable(b,g,f),m.push(g),k.push(f.name)):a.external("uniform",f)});_.each(g.attributes,function(b){a.external("attribute",b)});_.each(g.varyings,function(b){a.external("varying",b)});g=["_sg",b,h.name,e.owner().index].join("_");
h=h.compile(g,k,!0);a.add(b,g,m,h,i)}})(ShaderGraph);
...
a);if(!g[0])throw"Could not parse shader snippet. Must contain a void-returning function with in/outs: "+a;var m=a;_.each({parseAttribute:e,parseUniform:f,parseVarying:i},function(a,b){_.each(a,function(a){this[b](a);m=m.replace(a[0],"")}.bind(this))}.bind(this));m=m.replace(/^\s*;/,"");this.parseSignature(g[0]);this.body=m}}})(ShaderGraph);
